...
const Quiz = () => {
    ...
    const handleReset=()=>{
        ...
        setPreviousScore(()=>([...previousScore ,scoreCount]))
    ...
    return (
        <>
            <nav style={{ position: 'static', width: '100%', height: '50px', backgroundColor: '#4e034a' }}>
                <h1 id='heading'>QUIZ APP</h1></nav>

            <div className='AppBackground'>
                {score ? <div className='ScoreBoard'>
                    <h1>You Have score {scoreCount} out of 5</h1>
                    <div className='scoreActionButtons'>
                    <button style={{width:'100px'}} onClick={handleReset}>Retry</button>
                    <button style={{width:'230px'}} onClick={handlePreviousScore}>Show Previous Score</button>
                    </div>
                                                        </div> :
                    <div className='MainApp'>
                        <div className='QuestionSection'>
                            <h1 style={{ fontSize: '35px' }}>Question {currentquestion + 1}
                                <span style={{ fontSize: '18px' }}>/{questions.length}</span>
                            </h1>
